...
class LegacyProtocol extends Protocol {
    ...
    onSocketMessage(reader) {
        ...
        switch (messageId) {
            ...
            case 24:
                if (!this.gotKey || !this.settings.minionEnableERTPControls) break;
                this.connection.minionState = this.connection.minionState === 0 ? 1 : 0;
                break;
            case 25:
                if (!this.gotKey || !this.settings.minionEnableERTPControls) break;
                this.connection.minionState = this.connection.minionState === 2 ? 0 : 2;
                break;
            case 99:
                if (reader.length < 2)
    ...
    onNewWorldBounds(range, includeServerInfo) {
        ...
        if (includeServerInfo) {
            ...
            writeZTString(writer, `JoseFredo`, this.protocol);
        }
        this.send(writer.finalize());
    }
